...
namesResults <- c("RF"  ,     "RF2" ,     "LDA"     , "GBM"  ,    "KNN"  ,  
                  "RPART" ,   "GLM"  ,    "Combined",
                   "Type")
colnames(predDF4) <- namesResults
colnames(predDF4)
...
library(dplyr)
universe16 <- universeOrdered[1:16,-c(1:5)]
names <- row.names(universe16)
genes <- mutate(universe16, genes=names)#16X122
genes <- genes[,c(122,1:121)]
hgnc <- merge(genes, metaHGNC, by.x='genes', by.y='GENE_SYMBOL')
hgnc <- hgnc[,c(1,123:126,2:122)]#duplicates and some left out
hgnc <- hgnc[,c(1,4,3,5)]
hgnc <- hgnc[!duplicated(hgnc),]#11X4
sort(row.names(universe16))
# [1] "DNTT"    "FABP1"   "GRIK4"   "GRM8"    "INSM1"   "KLHDC4"  "KLK2"    "LIG4"    "MORC1"  
# [10] "POU3F2"  "RCVRN"   "SOX11"   "SUV39H1" "SYNGR3"  "TLX3"    "USP32P2"
sort(hgnc$genes)
# [1] "FABP1"   "GRIK4"   "GRM8"    "INSM1"   "KLHDC4"  "KLK2"    "LIG4"    "MORC1"   "POU3F2" 
# [10] "SOX11"   "USP32P2"

# DNTT, RCVRN, SUV39H1, SYNGR3, TLX3
metaHGNC[grep('DNTT', metaHGNC$GENE_SYMBOL),]#not in this data, from genenames.org
names <- colnames(hgnc)
DNTT <- data.frame(t(c('DNTT','DNA nucleotidylexotransferase', '+','hs|10q24.1')))
RCVRN <- data.frame(t(c('RCVRN','recoverin','-','hs|17p13.1')))
SUV39H1 <- data.frame(t(c('SUV39H1','suppresor of variegation 3-9 homolog1','+','hs|Xp11.23')))
SYNGR3 <- data.frame(t(c('SYNGR3','synaptogyrin 3','+','hs|16p13.3')))
TLX3 <- data.frame(t(c('TLX3','T cell leukemia homeobox 3','+','hs|5q35.1')))

others <- rbind(DNTT,RCVRN,SUV39H1,SYNGR3,TLX3)#5X4
colnames(others) <- names
hgnc <- rbind(hgnc,others)#16X4
colnames(hgnc) <- tolower(colnames(hgnc))
write.csv(hgnc, 'hgnc_least_universe.csv', row.names=FALSE)
universe_t <- data.frame(t(universe16)) # 121 X 16 genes

row.names(universe_t) #they are not ordered with the first 51 non-UL and next 70 UL samples

ul <- grep('UL',row.names(universe_t))#70
non <- universe_t[-ul,]#51X16
ul <- universe_t[ul,]#70X16
universe <- rbind(non,ul)#121X16
...
# add the TYPE field to the universal 16 genes having the most fold change
UNIVERSE <- cbind(TYPE, universe) #121X17

write.csv(UNIVERSE, 'least_universe_DE.csv', row.names=TRUE)

colnames(UNIVERSE)
# [1] "TYPE"    "HSPB1"   "DSTN"    "S100A6"  "CNN1"    "ACTG2"   "VIM"     "SPARCL1" "TPM2"   
# [10] "ACTA2"   "PCP4"    "TAGLN"   "DES"     "RAMP1"   "CYR61"   "UBC"     "ACTB"  
...
TOP16 <- TOP16[36,]
row.names(TOP16) <- 'TOP 10 Plus 6 DE 130'
colnames(TOP16) <- namesResults
...
row.names(DE16_130) <- 'Top 16 DE 130'
...
row.names(DE16_130_least) <- 'Bottom 16 DE 130'
...
row.names(FOLD16_130) <- 'Top 10 Plus 6 Fold 130'
...
row.names(majority10_130) <- 'Majority 10'
...
row.names(universe16_fold) <- 'Universe Top 16 Fold'
...
row.names(universe16_DE) <- 'Universe Top 16 DE'
...
row.names(universe16_DE_least) <- 'Universe Bottom 16 DE'
